...
function DPOIssuePage() {
  ...
  useEffect(() => {
    ...
    // Construct URL with date param if it exists
    let url = "http://127.0.0.1:8000/issues";
    
    // Handle Date Filtering (Convert dd-mm-yyyy to yyyy-mm-dd for Django)
    if (filters.date) {
        const parts = filters.date.split("-");
        if (parts.length === 3) {
            // FilterBar sends DD-MM-YYYY, API needs YYYY-MM-DD
